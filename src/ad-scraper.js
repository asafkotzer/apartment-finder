...
const scrape = (url, id, source) => {
	return new Promise((resolve, reject) => {
		...
		client
			...
			.execute(() =>
				Array.prototype.map
					.call(document.getElementsByClassName('v_checked'), el => el.parentElement)
					.filter(x => !!x)
					.map(x => x.innerText)
			)
			...
			.execute(() =>
				Array.prototype.map
					.call(document.querySelectorAll('.details_block_info > div > div'), el => el.innerText)
					.join()
			)
			...
			.execute(() =>
				Array.prototype.map
					.call(document.querySelectorAll('.imgesMuff img'), el => el.src)
					.map(x => x.replace('/s/', '/o/').replace('-s.jpg', '.jpg'))
			)
			.then(result => {
